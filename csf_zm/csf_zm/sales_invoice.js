    ...
    customer: function (frm) {
        setTimeout(function () {
            ...
            if (!frm.doc.tax_category) {
                frappe.call({
                    method: "csf_zm.custom_api.get_tax_category",
                    args: {
    ...
    make_sales_invoice_btn: function (frm) {
        if (frm.doc.docstatus == 1 && frm.doc.enabled_auto_create_delivery_notes == 1) {
            frm.add_custom_button(__('Create Delivery Note'),
                ...
                function () {
                    frappe.call({
                        method: "csf_zm.custom_api.create_delivery_note",
                        args: {
...
frappe.ui.form.on("Sales Invoice Item", {
    ...
    csf_zm_create_wtax_entry: (frm, cdt, cdn) => {
        frappe.call('csf_zm.custom_api.make_withholding_tax_gl_entries_for_sales', {
...
var validate_item_remaining_qty = function (frm, cdt, cdn) {
    ...
    frappe.call({
        method: 'csf_zm.custom_api.validate_item_remaining_qty',
        args: {
...
frappe.ui.keys.add_shortcut({
    ...
    action: () => {
        ...
        frappe.call({
            method: 'csf_zm.custom_api.get_item_info',
        ...
        new frappe.ui.form.SelectDialog({
            ...
            get_query() {
                return {
                    ...
                    query: "csf_zm.custom_api.get_item_prices_custom",
                }
            },
            return_field: "rate",
            action(selections) {
...
frappe.ui.keys.add_shortcut({
    ...
    action: () => {
        ...
        const item_row = locals["Sales Invoice Item"][current_doc];
        frappe.call({
            method: 'csf_zm.custom_api.get_item_prices',
            args: {
                item_code: item_row.item_code,
                currency: cur_frm.doc.currency,
                company: cur_frm.doc.company
