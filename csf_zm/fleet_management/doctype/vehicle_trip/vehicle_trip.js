...
frappe.ui.form.on('Vehicle Trip', {
    ...
    onload: function (frm) {
        ...
        frappe.model.with_doc(frm.doc.reference_doctype, frm.doc.reference_docname, function () {
            ...
            if (reference_doc.transporter_type == 'Sub-Contractor' || reference_doc.transporter_type == "Self Drive") {
                ...
                frm.toggle_display('driver', false);
                ...
                frm.toggle_display('driver', true);
                frm.toggle_display('trailer', true);
    ...
    validate_close: function (frm) {
        ...
        } else if (frm.doc.transporter_type == 'Bravo') {
    ...
    load_route_details: function (frm) {
        frappe.model.with_doc('Trip Route', frm.doc.main_route, function (frm) {
            var reference_route = frappe.model.get_doc('Trip Route', cur_frm.doc.main_route);
    ...
    load_return_route_steps: function (frm) {
        frappe.model.with_doc('Trip Route', frm.doc.return_route, function (frm) {
            var reference_route = frappe.model.get_doc('Trip Route', cur_frm.doc.return_route);
    ...
    route_steps_options: function (frm, cdt, cdn) {
        ...
        //Get trip location type options
        frappe.model.with_doc('Trip Location Type', row.doc.location_type, function (frm) {
            var reference = frappe.model.get_doc('Trip Location Type', row.doc.location_type);
    ...
    new_fund_request: function (frm) {
        ...
        if (frm.doc.main_requested_funds && frm.doc.main_requested_funds.length > 0) {
            ...
            if (new_main_request == true) {
                frappe.call({
                    method: "csf_zm.after_sales_services.doctype.requested_payments.requested_payments.request_funds",
                    args: {
    ...
    after_save: function (frm) {
        ...
        if (return_cancelled) {
            frappe.call({
                method: 'fleet_management.fleet_management.doctype.transport_assignment.transport_assignment.change_assignment_status',
                args: {
        ...
        if (to_save) {
            ...
            if (['Sub-Contractor', 'Self Drive'].indexOf(frm.doc.transporter_type) == -1) {
                frappe.call({
                    method: 'fleet_management.fleet_management.doctype.vehicle.vehicle.change_status',
                    args: {
...
frappe.ui.form.on('Route Steps Table', {
    form_render: function (frm, cdt, cdn) {
        frm.events.route_steps_options(frm, cdt, cdn);
    }
});

frappe.ui.form.on('Vehicle Trip Location Update', {
