...
def on_cancel_fees(doc, method):
    ...
    from csf_zm.bank_api import cancel_invoice
...
def make_withholding_tax_gl_entries_for_purchase(doc, method):
    ...
    for item in doc.items:
        if not item.withholding_tax_rate > 0 or item.csf_zm_wtax_jv_created == 1:
            continue
...
def enroll_all_students(self):
    ...
    if len(self.students) > 30:
        frappe.enqueue("csf_zm.custom_api.enroll_students", self=self)
        return "queued"
    else:
...
def enroll_students(self):
    ...
    Used in csf_zm.custom_api.enroll_students
    """
...
def auto_close_dn():
    ...
    customer_details = frappe.get_all("Customer", filters={"csf_zm_is_auto_close_dn": 1}, fields=["name", "csf_zm_close_dn_after"])
    ...
    for customer in customer_details:
        filters = {
            ...
            "status": ["!=", "Closed"],
            "posting_date": ["<", add_days(nowdate(), days=(-1 * customer.csf_zm_close_dn_after))],
        }
