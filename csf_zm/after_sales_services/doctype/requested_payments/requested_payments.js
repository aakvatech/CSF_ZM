...
frappe.ui.form.on('Requested Payments', {
	...
	make_payment: function() {
		frappe.model.open_mapped_doc({
			method: "csf_zm.after_sales_services.doctype.requested_payments.requested_payments.make_payment",
			frm: cur_frm
		})
	},
		...
		frappe.confirm(
			...
			function(){
				$.each(selected['requested_funds'], function(index, value){
					frappe.call({
						method: "csf_zm.after_sales_services.doctype.requested_payments.requested_payments.recommend_request",
						freeze: true,
						args: {
	...
	if(selected['requested_funds'])
	{
		frappe.confirm(
			...
			function(){
				$.each(selected['requested_funds'], function(index, value){
					frappe.call({
						method: "csf_zm.after_sales_services.doctype.requested_payments.requested_payments.approve_request",
						freeze: true,
						args: {
							request_doctype: "Requested Funds Details",
							request_docname: value,
	...
	if(selected['requested_funds'])
	{
		frappe.confirm(
			...
			function(){
				$.each(selected['requested_funds'], function(index, value){
					frappe.call({
						method: "csf_zm.after_sales_services.doctype.requested_payments.requested_payments.reject_request",
						freeze: true,
						args: {
							request_doctype: "Requested Funds Details",
							request_docname: value,
		...
		frappe.confirm(
			...
			function(){
				$.each(selected['accounts_approval'], function(index, value){
					frappe.call({
						method: "csf_zm.after_sales_services.doctype.requested_payments.requested_payments.accounts_approval",
						freeze: true,
						args: {
		...
		frappe.confirm(
			...
			function(){
				$.each(selected['accounts_approval'], function(index, value){
					reload = false;
					frappe.call({
						method: "csf_zm.after_sales_services.doctype.requested_payments.requested_payments.accounts_cancel",
						freeze: true,
						args: {
							request_doctype: "Requested Funds Accounts Table",
							request_docname: value,
